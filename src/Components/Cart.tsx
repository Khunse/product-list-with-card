...
export default function Cart({orderHandle} : {orderHandle : ()=> void})
{
    const {cartItem,setCartItem} = useCartContext();

    const removeItem = (w : Order)=>{
        if(cartItem.items.length == 0) return;
        else{
            const itemIn = cartItem.items.findIndex( iw => iw.itemName == w.itemName);
            if(itemIn >=0)
            {
                const itemslist = cartItem.items;
                const itemorder = cartItem.items[itemIn];
                const totalprice = cartItem.totalPrice - itemorder.totalItemPrice;
                itemslist.splice(itemIn,1);
                setCartItem({items: itemslist,totalPrice: totalprice});
            }
        }
    }
...
const handleClick = () => {
    ...
    return (
        <div className="bg-white rounded-md p-8 ycart lg:self-start lg:basis-1/3">
            <h1 className="ycart__title mb-5">Your Cart ({cartItem.items.reduce((total,w) => (total+w.quantity) ,0)})</h1>
            {
                ...
                <div className="flex flex-col items-center gap-5">
                    ...
                    <h2 className="text-md text-gray-500 font-semibold">Your added items will appear here</h2>
                </div>
                :
                <div className="">
                    {
                        cartItem.items.map(w => {
                            
                         return <div className="flex justify-between gap-2 items-center py-5  border-b">
                            <div className="flex flex-col gap-2">

                                    <h1 className="font-semibold">{w.itemName}</h1>
                                    <div className="flex gap-3">
                                        <span className="font-medium text-orange-600">{w.quantity}x</span>
                                        <span className="font-medium text-gray-500">@ ${w.itemPrice}</span>
                                        <span className="font-medium text-gray-600">${w.totalItemPrice.toFixed(1)}</span>
                                     </div>
                                </div>
                                <div className="border-2 border-gray-400 stroke-slate-400 p-1 rounded-full" onClick={()=> removeItem(w)}>
                    ...
                    <div className="py-5 flex justify-between items-center">
                        ...
                        <span className="font-bold text-2xl">${cartItem.totalPrice.toFixed(1)}</span>
                    </div>

                    <div className="flex gap-2 rounded-xl bg-gray-50 p-3 justify-center items-center text-sm">
                    ...
                    <button className="w-full rounded-3xl py-3 bg-orange-700 text-white font-semibold mt-7 hover:bg-orange-800" onClick={handleClick}>
                        Confirm Order
                    </button>
